# Project Brief: Inventory Service

## Overview

`@common-services/inventory` is a Vibe platform microservice in the
`common-services` monorepo. It is the HTTP API and data layer for inventory
...
1. **HTTP API** — Serve inventory routes through `xpref`
2. **Persistence** — PostgreSQL via `knexify` (`src/models/pool.ts`)
3. **Observability** — Publish logs with `@core/log-client` / RabbitMQ
4. **Domain APIs** — Inventory features defined under `memory-bank/feature/`
...
- Inventory domain APIs documented in `memory-bank/feature/`
