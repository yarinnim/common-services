# Product Context: Inventory Service

## Why this service exists

The platform needs a dedicated inventory microservice so stock, catalog, and
related operations stay isolated from logger, analytics, and other hubs.

Without this service, inventory rules would scatter across apps and there would
be no single API or database for inventory state.
...
1. Client calls a plural resource route (for example `/items`)
...
This path is for local checks only, not inventory business logic.

## User experience goals

### Developers

- Clear feature modules under `src/services/<feature>/`
...
- **Logger**: inventory publishes to `LOG_EXCHANGE` (`logger-service`)
- **Analytics**: separate hub; inventory does not store analytics events
