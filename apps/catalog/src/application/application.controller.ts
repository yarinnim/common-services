...
/**
 * Soft-deletes an application and returns the previous public row.
 *
 * @example
 * removeAndReturn(targetApplication);
 */
const removeAndReturn = (application: NonNullable<
  ApplicationDetailRequest['targetApplication']
>) => removeApplication(application.id).then(() => application);
...
export const deleteAction = (
  ...
  .then(() => {
    const { targetApplication } = req;
    if (!targetApplication) throw new Error('Application not found.');
    return removeAndReturn(targetApplication);
  })
  .then((application) => res.json(application))
  .catch((error: Error) => {
    const { message } = error;
