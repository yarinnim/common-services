...
`@common-services/payment` is a Vibe platform microservice in the
`common-services` monorepo. It is the HTTP API and data layer for payment
...
- Payment domain APIs documented in `memory-bank/feature/`
...
- Feature modules under `src/<feature>/` (router, controller, service,
  middleware)
